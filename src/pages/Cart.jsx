...
export default function Cart() {
  const { cart, addToCart, removeFromCart, decrementCartItem, clearCart, data } = useAppStore();
  ...
  const getWhatsApp = () => {
    const lines = cart.map(i => `• ${i.title} (x${i.quantity})`).join('\n');
    const msg = `Olá Gabz, fiz o pedido no site e paguei via PIX.\n\nPedido:\n${lines}\n\nTotal: R$ ${total.toFixed(2)}\n\nQuando podemos começar?`;
  ...
  return (
    <div className="container">
          ...
          {cart.map(item => (
            <div key={item.id} className="cart-item-card">
              ...
              <div className="cart-item-right">
                <div className="cart-item-qty-controls" style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,0.05)', padding: '4px 8px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.08)' }}>
                  <button
                    onClick={() => decrementCartItem(item.id)}
                    style={{ background: 'none', border: 'none', color: '#ff4b4b', padding: '2px 8px', fontSize: '1rem', cursor: 'pointer', boxShadow: 'none', width: 'auto', minWidth: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                    title="Diminuir quantidade"
                  >
                    -
                  </button>
                  <span style={{ fontFamily: 'Russo One', fontSize: '0.9rem', color: '#fff', minWidth: 16, textAlign: 'center' }}>
                    {item.quantity}
                  </span>
                  <button
                    onClick={() => addToCart(item)}
                    style={{ background: 'none', border: 'none', color: '#00e676', padding: '2px 8px', fontSize: '1rem', cursor: 'pointer', boxShadow: 'none', width: 'auto', minWidth: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
      ...
      {/* PIX Modal */}
      {showPix && (
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && handleClosePix()}>
          <div className="modal-content">
            <button className="modal-close" onClick={handleClosePix}><X size={18} /></button>
              ...
              <>
                ...
                <a href={getWhatsApp()} target="_blank" rel="noreferrer" style={{ display: 'block' }}>
                  <button
                    ...
                    style={{ width: '100%', background: 'linear-gradient(180deg,#25D366 0%,#128C7E 100%)', boxShadow: '0 6px 0 #075E54, 0 10px 25px rgba(37,211,102,0.35)' }}
                  >
